...
§0「文書の役割」と §1「スライス順序表」を読み、§2 Phase 0（共通基盤）
から順に着手する。各スライスの定義は §3、ADR は §4、未確定の残課題は
§5 にある。

実装途中で曖昧さが出たら勝手に判断せず、実装計画 §5「残課題」と
...
| `implementation-plan.md` | スライス順序、Phase 0、各スライスのスコープ・前提・タスク分割案・DoD、ADR、残課題 | 実装の進め方 |
