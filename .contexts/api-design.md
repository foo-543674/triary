...
| `exceeds_max_blocks` | セッションの種目ブロック数上限を超える | `blocks` |
| `missing_required_measurement` | 種目が必須としている計測項目がセットに欠けている | `reps` / `weight_kg` / `duration_seconds` のいずれか（複数欠けている場合は `errors[]` を複数要素にして要素ごとに該当 field を入れる） |
...
- エラー:
  ...
  - 400 `exceeds_max_blocks` on `blocks`（1 セッション最大 50 ブロック制限超過、spec §主要な前提）
...
- エラー:
  ...
  - 400 `missing_required_measurement` on `reps` / `weight_kg` / `duration_seconds`（種目が必須としている計測項目がセットに欠けている、§1.6 の専用コード。複数欠けている場合は `errors[]` を複数要素で返す）
...
- エラー:
  - 404 `not_found`
  - 400 on `reps` / `weight_kg` / `duration_seconds` / `interval_seconds`: `out_of_range`
  - 400 `missing_required_measurement` on `reps` / `weight_kg` / `duration_seconds`（編集後に種目の必須計測項目が欠ける場合、§1.6 の専用コード。複数欠けている場合は `errors[]` を複数要素で返す）
...
- 備考:
  ...
  - `previous_summary` は spec US-10「前回の数値を一目で把握できる」UI
    ...
    `sets` は直近 1 ブロックの **全セット** を `Set[]` で返す（`items[].sets`
    と同形）。サーバー側で要約・件数制限はしない（spec US-10 受け入れ
    基準「直近の種目ブロックのセット内容」に従う）
