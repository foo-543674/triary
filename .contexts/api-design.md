...
| 認証必須エンドポイント | `/api/v1/auth/login`, `/api/v1/users` (signup), `/api/v1/health` 以外すべて |
| 未認証アクセス | `401 Unauthorized` + `{"error":"unauthenticated", ...}` |
| ログアウト | `POST /api/v1/auth/logout` でサーバー側の `user_sessions` 行を削除し、`Set-Cookie: triary_session=; Max-Age=0` を返す |
